...
export async function fetchYandexDailyEarnings(dateStr) {
  ...
  const perDriver = new Map(); // yandex_driver_id -> { cash, card, yandexCommission }
  let offset = 0;
  const limit = 500;
  ...
  while (true) {
    ...
    for (const t of page) {
      ...
      if (!perDriver.has(driverId)) perDriver.set(driverId, { cash: 0, card: 0, yandexCommission: 0 });
  ...
  return { perDriver, debugSample, range: { fromISO, toISO } };
}
